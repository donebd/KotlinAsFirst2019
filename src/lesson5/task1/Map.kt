...
fun canBuildFrom(chars: List<Char>, word: String): Boolean {
    for (i in 0 until word.length) if (word[i].toLowerCase() !in chars.toMutableList().map { it.toLowerCase() }) return false
    return true
}
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    for (i in 0..list.size - 2)
        for (j in i + 1 until list.size) if (list[i] + list[j] == number) return Pair(i, j)
    return Pair(-1, -1)
}
...
fun bagPacking(treasures: Map<String, Pair<Int, Int>>, capacity: Int): Set<String> {
    ...
    while (treasures.filter { it.value.first <= freeSlot && it.key !in answer }.isNotEmpty()) {
        ...
        answer.plusAssign(setOf(a.key))
        a = treasures.filter { it.value.first <= freeSlot && it.key !in answer }
            .maxBy { it.value.second.toDouble() / it.value.first }
    }
    return answer
}
